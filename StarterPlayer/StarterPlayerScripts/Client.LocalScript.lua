...
function UpdateFilter()
    local v364 = #v_u_360 > 0
    local v365 = v_u_359[v_u_361]
    for _, v366 in pairs(v_u_329:GetChildren()) do
        if v366:IsA("TextButton") then
            if v364 then
                local v367 = v_u_357[v_u_360]
                ...
                elseif v368:lower():find(v_u_360) then
                    v366.Visible = true
                else
                    v366.Visible = false
                end
...
v_u_355.FocusLost:Connect(function(_)
    v_u_360 = v_u_355.Text:lower()
    UpdateFilter()
end)
v_u_356.MouseButton1Click:Connect(function()
    v_u_361 = v_u_361 % #v_u_359 + 1
    local v373 = v_u_359[v_u_361]
    v_u_356.Text = v_u_358[v373] or v373
    UpdateFilter()
end)
...
local function v_u_388(p380, p381, p382)
    ...
    p380.Name = p380:GetAttribute(v_u_359[v_u_361])
    if #v_u_360 > 0 then
        if v383:lower():find(v_u_360) then
            p380.Visible = true
            return
        end
        p380.Visible = false
...
local function v_u_661()
    local v651 = #v_u_645 > 0 and true or v_u_648
    local v652 = v_u_644[v_u_646]
    for _, v653 in pairs(v_u_583:GetChildren()) do
        if v653:IsA("TextButton") then
            if v651 then
                local v654 = v_u_643[v_u_645]
                ...
                elseif v655:lower():find(v_u_645) then
                    v653.Visible = true
                else
                    v653.Visible = false
                end
...
v_u_639.FocusLost:Connect(function(_)
    v_u_645 = v_u_639.Text:lower()
    v_u_661()
end)
v_u_640.MouseButton1Click:Connect(function()
    v_u_646 = v_u_646 % #v_u_644 + 1
    local v662 = v_u_644[v_u_646]
    v_u_640.Text = v_u_647[v662] or v662
    v_u_661()
end)
...
local function v_u_671(p663, p664, p665)
    ...
    p663.Name = p663:GetAttribute(v_u_644[v_u_646])
    if #v_u_645 > 0 then
        if v666:lower():find(v_u_645) then
            p663.Visible = true
            return
        end
        p663.Visible = false
...
local v_u_1019 = v909
local v_u_1020 = v_u_857
local v_u_1021 = v_u_910
...
table.insert(v_u_6, function()
    if getOrSetCooldowns("FlickerSconces", 0.25) then
        ...
        if v_u_1019 then
            ...
            if v1041 and (v1041.Position - v_u_1021).Magnitude > 100 then
                print(" far away now ")
                v_u_1019 = false
            end
        end
